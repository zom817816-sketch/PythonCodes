...
# ────────────────────────────────────────────────────────
# 1. 每种硬币可以无限使用 → 完全背包
# 2. 只有一个维度（金额 amount）→ 一维 DP
# 3. 目标是求组合数（+=）而非最大值（max）
#
# 为什么循环顺序如此重要？
# ────────────────────────────────────────────────────────
# 本题要求的是组合数（combinations），而非排列数（permutations）。
#
# 外层遍历硬币、内层遍历金额（正确，计算组合数）：
#    - 每种硬币按顺序依次被考虑
#    - 先处理 coin=1，再处理 coin=2
#    - 这样 {1,2} 和 {2,1} 只会被计算一次（因为 2 总是在 1 之后被考虑）
#
# 外层遍历金额、内层遍历硬币（错误，计算排列数）：
#    - 对于每个金额，所有硬币都会被考虑
#    - amount=3 时，coin=1 和 coin=2 都会被尝试
#    - {1,2} 和 {2,1} 会被算成两种不同的方案
#
# 与 0-1 背包的区别：
# ────────────────────────────────────────────────────────
# 0-1 背包（物品只能用一次）：内层从后往前遍历
#    for j in range(capacity, weight - 1, -1):
#        dp[j] = max(dp[j], dp[j - weight] + value)
#
# 完全背包（物品可以无限用）：内层从前往后遍历
#    for i in range(weight, capacity + 1):
#        dp[i] += dp[i - weight]
#
# 时间复杂度对比：
# ────────────────────────────────────────────────────────
# - 回溯算法：O(S^amount) — 指数级，不可接受
# - 记忆化搜索：O(n×amount) — 多项式级
# - 动态规划：O(n×amount) — 多项式级，空间最优
#
# 常见错误：
# ────────────────────────────────────────────────────────
# 1. 循序顺序写反：外层遍历金额、内层遍历硬币（计算排列数）
# 2. 记忆化搜索缺少 index 参数（计算排列数）
# 3. base case 写成 remaining in coins（跳过其他组合）
